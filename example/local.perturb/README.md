...
good_group1 = [
	...
	for gene in genes_group1
	if gene in ensemble_dictionary
]
good_group2 = [
	ensemble_dictionary[gene]
	for gene in genes_group2
	if gene in ensemble_dictionary
]
...
def run_perturb(model_path, dataset_path, out_dir, num_embs=None):
	...
	# Filter genes that exist in the token_dictionary
	good_genes_final = [
		gene for gene in good_genes if gene in list(token_dictionary.keys())
	]
...
| 13408 | 4242 | CD274 | ENSG00000120217 | 13395   | FXYD7              | ENSG00000221946       | 0.9513469934463501  | 0.00003063678741455078     | 2 |
