...
from transformers import (BertForMaskedLM, BertForSequenceClassification, BertForTokenClassification, BitsAndBytesConfig)
import env
...
def filter_by_dict(data, filter_data, nproc):
	...
	if len(data) == 0:
		logger.error("No spots remain after filtering. Check filtering criteria.")
		raise
	return data
def filter_data_by_tokens(filtered_input_data, tokens, nproc):
	def if_has_tokens(example):
		...
		if len(set(example["input_ids"]).intersection(tokens)) == len(tokens):
			for token in tokens:
				if token in example["input_ids"]:
					indices = [i for i,x in enumerate(example["input_ids"]) if x==token]
					if len(indices)<2:
						return False
					#else:
					#	print("Get here", len(indices))
				else:
					return False
			return True
		else:
...
def logging_filtered_data_len(filtered_input_data, filtered_tokens_categ):
	if len(filtered_input_data) == 0:
		logger.error(f"No spots in dataset contain {filtered_tokens_categ}.")
		raise
	else:
		logger.info(f"# spots with {filtered_tokens_categ}: {len(filtered_input_data)}")
def filter_data_by_tokens_and_log(filtered_input_data, tokens, nproc, filtered_tokens_categ):
	...
	return filtered_input_data
def filter_data_by_start_state(filtered_input_data, spot_states_to_model, nproc):
	...
	start_state = spot_states_to_model["start_state"]
	if start_state not in state_values:
		logger.error(f"Start state {start_state} is not present in the dataset's {state_key} attribute.")
		raise
	# filter for start state spots
	...
	return filtered_input_data
def slice_by_inds_to_perturb(filtered_input_data, spot_inds_to_perturb):
	if spot_inds_to_perturb["start"] >= len(filtered_input_data):
		logger.error("spot_inds_to_perturb['start'] is larger than the filtered dataset.")
		raise
	if spot_inds_to_perturb["end"] > len(filtered_input_data):
		logger.warning("spot_inds_to_perturb['end'] is larger than the filtered dataset. Setting to the end of the filtered dataset.")
		spot_inds_to_perturb["end"] = len(filtered_input_data)
	filtered_input_data = filtered_input_data.select([i for i in range(spot_inds_to_perturb["start"], spot_inds_to_perturb["end"])])
	return filtered_input_data
# load model to GPU
def load_model(model_type, num_classes, model_directory, mode):
	output_hidden_states = (mode == "eval")
	# Model class selection
	model_classes = {"Pretrained": BertForMaskedLM, "GeneClassifier": BertForTokenClassification, "SpotClassifier": BertForSequenceClassification}
	model_class = model_classes.get(model_type)
	if not model_class:
		raise ValueError(f"Unknown model type: {model_type}")
	# Model loading
	model_args = {"pretrained_model_name_or_path": model_directory, "output_hidden_states": output_hidden_states, "output_attentions": False}
...
def measure_length(example):
	...
	return example
def downsample_and_sort(data, max_num_spots):
	num_spots = len(data)
	# if max number of spots is defined, then shuffle and subsample to this max number
	if max_num_spots is not None:
		if num_spots > max_num_spots:
			data = data.shuffle(seed=42)
			num_spots = max_num_spots
	data_subset = data.select([i for i in range(num_spots)])
	# sort dataset with largest spot first to encounter any memory errors earlier
	data_sorted = data_subset.sort("length", reverse=True)
	return data_sorted
def get_possible_states(spot_states_to_model):
	possible_states = []
	for key in ["start_state", "goal_state"]:
		possible_states += [spot_states_to_model[key]]
	possible_states += spot_states_to_model.get("alt_states", [])
	return possible_states
def forward_pass_single_spot(model, example_spot, layer_to_quant):
	example_spot.set_format(type="torch")
	input_data = example_spot["input_ids"]
	with torch.no_grad():
...
def delete_indices(example):
	...
	example["length"] = len(example["input_ids"])
	return example
# for genes_to_perturb = "all" where only genes within spot are overexpressed
...
def overexpress_indices_special(example):
	...
	example["length"] = len(example["input_ids"])
	return example
# for genes_to_perturb = list of genes to overexpress that are not necessarily expressed in spot
def overexpress_tokens(example, max_len, special_token):
	# -100 indicates tokens to overexpress are not present in rank value encoding
	if example["perturb_index"] != [-100]:
		example = delete_indices(example)
	if special_token:
		[example["input_ids"].insert(1, token) for token in example["tokens_to_perturb"][::-1]]
	else:
...
def remove_indices_from_emb_batch(emb_batch, list_of_indices_to_remove, gene_dim):
	output_batch_list = [remove_indices_from_emb(emb_batch[i, :, :], idxes, gene_dim - 1) for i, idxes in enumerate(list_of_indices_to_remove)]
	# add padding given genes are sometimes added that are or are not in original spot
	batch_max = max([emb.size()[gene_dim - 1] for emb in output_batch_list])
	output_batch_list_padded = [pad_xd_tensor(emb, 0.000, batch_max, gene_dim - 1) for emb in output_batch_list]
...
def remove_perturbed_indices_set(emb, perturb_type: str, indices_to_perturb: List[List], tokens_to_perturb: List[List], 
	original_lengths: List[int], input_ids=None):
	if perturb_type == "overexpress":
		num_perturbed = len(tokens_to_perturb)
		if num_perturbed == 1:
			indices_to_perturb_orig = [idx if idx != [-100] else [None] for idx in indices_to_perturb]
...
def make_perturbation_batch(example_spot, perturb_type, tokens_to_perturb, anchor_token, combo_lvl, num_proc) -> tuple[Dataset, List[int]]:
	if combo_lvl == 0 and tokens_to_perturb == "all":
		if perturb_type in ["overexpress", "activate"]:
			range_start = 1
		elif perturb_type in ["delete", "inhibit"]:
			range_start = 0
		indices_to_perturb = [[i] for i in range(range_start, example_spot["length"][0])]
	# elif combo_lvl > 0 and anchor_token is None:
	## to implement
	elif combo_lvl > 0 and (anchor_token is not None):
		example_input_ids = example_spot["input_ids"][0]
		anchor_index = example_input_ids.index(anchor_token[0])
		indices_to_perturb = [sorted([anchor_index, i]) if i != anchor_index else None for i in range(example_spot["length"][0])]
		...
		for token in tokens_to_perturb:
			if token in example_input_ids:
				...
				indices_to_perturb.append(indices)
			else:
	...
	if combo_lvl > 0 and (anchor_token is None):
		if tokens_to_perturb != "all":
			...
			all_indices = [[i] for i in range(example_spot["length"][0])]
	...
	perturbation_dataset = Dataset.from_dict({"input_ids": example_spot["input_ids"] * length, "perturb_index": indices_to_perturb})
	if length < 400:
		num_proc_i = 1
	else:
		num_proc_i = num_proc
...
def make_perturbation_batch_special(example_spot, perturb_type, tokens_to_perturb, anchor_token, combo_lvl, num_proc
	...
	if combo_lvl == 0 and tokens_to_perturb == "all":
		...
		indices_to_perturb = [[i] for i in range(range_start, example_spot["length"][0] - 1)]
	# elif combo_lvl > 0 and anchor_token is None:
	## to implement
	elif combo_lvl > 0 and (anchor_token is not None):
		example_input_ids = example_spot["input_ids"][0]
		...
		indices_to_perturb = [sorted([anchor_index, i]) if i != anchor_index else None
			for i in range(1, example_spot["length"][0] - 1)]
		indices_to_perturb = [item for item in indices_to_perturb if item is not None]
	else:
		example_input_ids = example_spot["input_ids"][0]
		#indices_to_perturb = [[example_input_ids.index(token)] if token in example_input_ids else None
	...
	if combo_lvl > 0 and (anchor_token is None):
		if tokens_to_perturb != "all":
			...
			all_indices = [[i] for i in range(1, example_spot["length"][0] - 1)]  # Exclude CLS and EOS tokens
			all_indices = [index for index in all_indices if index not in indices_to_perturb]
			indices_to_perturb = [[[j for i in indices_to_perturb for j in i], x] for x in all_indices]
	length = len(indices_to_perturb)
	perturbation_dataset = Dataset.from_dict({"input_ids": example_spot["input_ids"] * length, "perturb_index": indices_to_perturb})
	...
	if perturb_type == "delete":
		...
		perturbation_dataset = perturbation_dataset.map(overexpress_indices_special, num_proc=num_proc_i)
	perturbation_dataset = perturbation_dataset.map(measure_length, num_proc=num_proc_i)
	return perturbation_dataset, indices_to_perturb
# original spot emb removing the activated/overexpressed/inhibited gene emb
...
def make_comparison_batch(original_emb_batch, indices_to_perturb, perturb_group):
	all_embs_list = []
	# if making comparison batch for multiple perturbations in single spot
	if perturb_group is False:
		# squeeze if single spot
	...
	# if making comparison batch for single perturbation in multiple spots
...
def pad_list(input_ids, pad_token_id, max_len):
	input_ids = np.pad(input_ids, (0, max_len - len(input_ids)), mode="constant", constant_values=pad_token_id)
	return input_ids
def pad_xd_tensor(tensor, pad_token_id, max_len, dim):
	...
	return torch.nn.functional.pad(tensor, pad=pad_config, mode="constant", value=pad_token_id)
def pad_tensor(tensor, pad_token_id, max_len):
	tensor = torch.nn.functional.pad(tensor, pad=(0, max_len - tensor.numel()), mode="constant", value=pad_token_id)
	return tensor
def pad_2d_tensor(tensor, pad_token_id, max_len, dim):
	...
	tensor = torch.nn.functional.pad(tensor, pad=pad, mode="constant", value=pad_token_id)
	return tensor
...
def pad_tensor_list(tensor_list, dynamic_or_constant, pad_token_id, model_input_size, dim=None, padding_func=None):
	...
	if dynamic_or_constant == "dynamic":
		...
		logger.warning("If padding style is constant, must provide integer value. "
	...
	if dim is None:
		...
		tensor_list = [padding_func(tensor, pad_token_id, max_len, dim) for tensor in tensor_list]
...
def gen_attention_mask(minibatch_encoding, max_len=None):
	...
	attention_mask = [[1] * original_len + [0] * (max_len - original_len) if original_len <= max_len
		else [1] * max_len for original_len in original_lens]
	return torch.tensor(attention_mask, device="cuda")
# get spot embeddings excluding padding
def mean_nonpadding_embs(embs, original_lens, dim=1):
	...
	return mean_embs
# get spot embeddings when there is no padding
def compute_nonpadded_spot_embedding(embs, spot_emb_style):
	if spot_emb_style == "mean_pool":
		return torch.mean(embs, dim=embs.ndim - 2)
# quantify shifts for a set of genes
def quant_cos_sims(perturbation_emb, original_emb, spot_states_to_model, state_embs_dict, emb_mode="gene"):
	if emb_mode == "gene":
		cos = torch.nn.CosineSimilarity(dim=2)
	elif emb_mode == "spot":
		cos = torch.nn.CosineSimilarity(dim=1)
	# if emb_mode == "gene", can only calculate gene cos sims
	# against original spot
	if spot_states_to_model is None or emb_mode == "gene":
		cos_sims = cos(perturbation_emb, original_emb).to("cuda")
	elif spot_states_to_model is not None and emb_mode == "spot":
		possible_states = get_possible_states(spot_states_to_model)
		cos_sims = dict(zip(possible_states, [[] for _ in range(len(possible_states))]))
		for state in possible_states:
			#required to move to cuda here
			cos_sims[state] = cos_sim_shift(original_emb, perturbation_emb, state_embs_dict[state].to("cuda"), cos)
	return cos_sims
# calculate cos sim shift of perturbation with respect to origin and alternative spot
...
def concatenate_cos_sims(cos_sims):
	if isinstance(cos_sims, list):
		...
		return cos_sims
def write_perturbation_dictionary(cos_sims_dict: defaultdict, o_path_prefix: str):
	with open(f"{o_path_prefix}_raw.pickle", "wb") as fp:
		pickle.dump(cos_sims_dict, fp)
def tensor_list_to_pd(tensor_list):
	tensor = torch.cat(tensor_list).cpu().numpy()
	df = pd.DataFrame(tensor)
	return df
def validate_spot_states_to_model(spot_states_to_model):
	if spot_states_to_model is not None:
		if len(spot_states_to_model.items()) == 1:
			logger.warning("The single value dictionary for spot_states_to_model will be "
				...
				"in the spot_states_to_model dictionary for future use. "
				"For example, spot_states_to_model={"
				"'state_key': 'disease', "
				"'start_state': 'dcm', "
				"'goal_state': 'nf', "
				"'alt_states': ['hcm', 'other1', 'other2']}")
			for key, value in spot_states_to_model.items():
			...
			state_values = flatten(list(spot_states_to_model.values()))
			spot_states_to_model = {"state_key": list(spot_states_to_model.keys())[0], "start_state": state_values[0][0],
				"goal_state": state_values[1][0], "alt_states": state_values[2:][0]}
		elif set(spot_states_to_model.keys()).issuperset({"state_key", "start_state", "goal_state"}):
			if ((spot_states_to_model["state_key"] is None) or (spot_states_to_model["start_state"] is None) 
				or (spot_states_to_model["goal_state"] is None)):
				logger.error("Please specify 'state_key', 'start_state', and 'goal_state' in spot_states_to_model.")
				raise
			if (spot_states_to_model["start_state"]== spot_states_to_model["goal_state"]):
				logger.error("All states must be unique.")
				raise
			if "alt_states" in set(spot_states_to_model.keys()):
				if spot_states_to_model["alt_states"] is not None:
					if not isinstance(spot_states_to_model["alt_states"], list):
						logger.error("spot_states_to_model['alt_states'] must be a list (even if it is one element).")
						raise
					if len(spot_states_to_model["alt_states"]) != len(set(spot_states_to_model["alt_states"])):
						logger.error("All states must be unique.")
						raise
			else:
				spot_states_to_model["alt_states"] = []
		else:
			logger.error("spot_states_to_model must only have the following four keys: "
