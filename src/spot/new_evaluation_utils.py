...
from sklearn.metrics import (ConfusionMatrixDisplay, accuracy_score, auc, confusion_matrix, f1_score, roc_curve)
from tqdm.auto import trange
import env
TOKEN_DICTIONARY_FILE = env.get_token_dictionary_file()
from new_emb_extractor import make_colorbar
logger = logging.getLogger(__name__)
def preprocess_classifier_batch(spot_batch, max_len, label_name):
	if max_len is None:
		max_len = max([len(i) for i in spot_batch["input_ids"]])
	# load token dictionary (Ensembl IDs:token)
	with open(TOKEN_DICTIONARY_FILE, "rb") as f:
		gene_token_dict = pickle.load(f)
	def pad_label_example(example):
		example[label_name] = np.pad(example[label_name], (0, max_len - len(example["input_ids"])), mode="constant", constant_values=-100)
		example["input_ids"] = np.pad(example["input_ids"], (0, max_len - len(example["input_ids"])), mode="constant", constant_values=gene_token_dict.get("<pad>"))
		example["attention_mask"] = (example["input_ids"] != gene_token_dict.get("<pad>")).astype(int)
		return example
	padded_batch = spot_batch.map(pad_label_example)
	return padded_batch
...
def classifier_predict(model, classifier_type, evalset, forward_batch_size):
	...
	for i in trange(0, evalset_len, forward_batch_size):
		...
		padded_batch = preprocess_classifier_batch(batch_evalset, max_evalset_len, label_name)
	...
	enable_progress_bar()
	logits_by_spot = torch.cat(predict_logits)
	last_dim = len(logits_by_spot.shape) - 1
	all_logits = logits_by_spot.reshape(-1, logits_by_spot.shape[last_dim])
	labels_by_spot = torch.cat(predict_labels)
	all_labels = torch.flatten(labels_by_spot)
...
def get_metrics(y_pred, y_true, logits_list, num_classes, labels):
	...
	if num_classes == 2:
		...
		tpr_wt = len(tpr)
		roc_auc = auc(fpr, tpr)
		roc_metrics = {"fpr": fpr, "tpr": tpr, "interp_tpr": interp_tpr, "auc": roc_auc, "tpr_wt": tpr_wt}
...
# plot ROC curve
def plot_ROC(roc_metric_dict, model_style_dict, title, o_dir, o_prefix):
	fig = plt.figure()
	...
	for model_name in roc_metric_dict.keys():
		...
		plt.plot(mean_fpr, mean_tpr, color=color, linestyle=linestyle, lw=lw, label=label)
	...
	output_file = (Path(o_dir) / f"{o_prefix}_roc").with_suffix(".pdf")
	plt.savefig(output_file, bbox_inches="tight")
	plt.show()
def plot_confusion_matrix(conf_mat_df, title, o_dir, o_prefix, custom_class_order):
	fig = plt.figure()
	...
	display = ConfusionMatrixDisplay(confusion_matrix=conf_mat, display_labels=display_labels)
	display.plot(cmap="Blues", values_format=".2g")
	plt.title(title)
	plt.show()
	output_file = (Path(o_dir) / f"{o_prefix}_conf_mat").with_suffix(".pdf")
...
def plot_predictions(predictions_df, title, o_dir, o_prefix, kwargs_dict):
	sns.set(font_scale=2)
	...
	predict_colors = pd.DataFrame(pd.Series(predict_colors_list, index=predict_label_list), columns=["predicted"])
	default_kwargs_dict = {"row_cluster": False, "col_cluster": False, "row_colors": label_colors, "col_colors": predict_colors,
		"linewidths": 0, "xticklabels": False, "yticklabels": False, "center": 0, "cmap": "vlag"}
	...
	for label_color in list(label_color_dict.keys()):
		...
		g.ax_col_dendrogram.legend(title=f"{title}", loc="lower center", ncol=4, bbox_to_anchor=(0.5, 1), facecolor="white")
	output_file = (Path(o_dir) / f"{o_prefix}_pred").with_suffix(".pdf")
	plt.savefig(output_file, bbox_inches="tight")
